...
`6cdd803`, is recorded with the command that read it. Three were already right. **Two
things remain, and both are a single click.**
...
| #   | What                                            | State                                  | Action          |
| --- | ----------------------------------------------- | -------------------------------------- | --------------- |
| 1   | Actions → Workflow permissions **read + write** | ❌ `read`                              | **do this**     |
...
| 5   | The app's _Release after merge_ switch          | ❌ **on** for the Task Manager project | **do this**     |

Items 1 and 5 are both required, and neither is optional in the sense of "the pipeline is
...
The repository is currently read-only:
...
can only ever _narrow_ what the repository allows — it can never raise it. Left as it is, that
declaration grants nothing, and the `version` job fails at its `git push` with a 403 that
reads like a bad credential rather than a repository setting.

> **Leave _Allow GitHub Actions to create and approve pull requests_ unchecked.** It sits
> directly under the radio button you are changing, and nothing here needs it — no workflow
> opens a pull request. It is the one genuinely dangerous half of that settings page.

Widening the default does not quietly widen the three workflows: each declares its own
`permissions:` block and so keeps narrowing itself to what it needs (`ci.yml` stays
read-only, `deploy.yml` takes `packages: write` + `id-token: write`, `release.yml` takes
`contents: write`). What changes is the default a _future_ workflow would inherit if it
forgot to declare one.

**Verify** — the same command should answer `write`:
...
Both remaining clicks should happen **before this branch merges into `development`**, because
the merge is the first real run of everything:
...
1. **Turn the app switch off (5).** If it is still on when the branch merges, the orchestrator
   ...
   first run, before anyone has seen it work.
2. **Set workflow permissions (1).** If this is still `read` at the merge, `release.yml` runs
   and dies in the `version` job. That is recoverable — fix the setting, then Actions →
   **Release** → _Run workflow_ → branch `development`, which is a plain re-run because
   nothing was created before the failure. But it is a worse first impression than a click.
