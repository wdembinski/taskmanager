...
That section prices the Container Apps + socket.io shape the plan currently
targets; this note lays it next to the alternatives raised while sizing it,
so the choice is visible in one place before it gets locked into the plan.
...
Scenario B into a second replica and a Redis backplane. Rows 4–6 move the
...
"raw pub/sub" vs. "hubs and groups," not budget. Row 6 is the floor: no
realtime infrastructure at all, traded for up-to-30-second staleness instead
of a live push.
