...
| The web says no desktop app has ever synced | Ask the desktop, not the deployment: **Settings → Cloud → Test connection** walks the whole chain — address, sign-in, the master switch, this machine's own `POST /v1/sync`, and whether the server then lists it among `BoardResponse.clients` — and names the rung that fails. The three that used to be invisible: cloud sync switched off, an account granted `read` but not `write` (the guard authorizes per method, so reads work and every sync 403s), and more than one replica. |
