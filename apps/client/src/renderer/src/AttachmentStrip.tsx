...
import { attachmentUrl, type TaskAttachment } from '@shared/attachments';
...
export function AttachmentStrip({
  ...
  async function add(paths: string[]): Promise<void> {
    ...
    try {
      const all = await window.api.invoke('attachment:add', taskId, paths);
  ...
  async function pick(): Promise<void> {
    setError(null);
    try {
      await add(await window.api.invoke('attachment:pick'));
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
    }
  }
  ...
  async function remove(id: string): Promise<void> {
    ...
    setError(null);
    try {
      await window.api.invoke('attachment:remove', id);
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
    } finally {
      setBusy(false);
  ...
  async function open(id: string): Promise<void> {
    setError(null);
    try {
      const failure = await window.api.invoke('attachment:open', id);
